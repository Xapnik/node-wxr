...
module.exports = Importer = (function() {
  ...
  Importer.prototype.addPost = function(post) {
    var cat, catCDATA, category, contentEncoded, contentEncodedCDATA, creator, date, id, description, excerptEncoded, excerptEncodedCDATA, i, item, len, name, parent, ref, ref1, results, status, title, type;
    ...
    status = item.ele("wp:status", {}).txt(post.status ? post.status : "publish");
    parent = item.ele("wp:post_parent", {}).txt(0);
    type = item.ele("wp:post_type", {}).txt("post");
    creator = item.ele("dc:creator", {}).txt(post.author ? post.author : "admin");
  ...
  Importer.prototype.addAttachment = function(options) {
    var creator, date, description, contentEncoded, contentEncodedCDATA, excerptEncoded, excerptEncodedCDATA, item, parent, status, title, type, postID, commentStatus, pingStatus, postName, menuOrder, isSticky, attachedFileMeta, attachementImageAltMeta;
    if (options.attachmentURL) {
      item = this.channel.ele("item");
      date = item.ele("wp:post_date", {}).txt(normalizeDate(options.date));
      title = item.ele("title", {}).txt(options.title ? options.title : "Default title for attachment");
      creator = item.ele("dc:creator", {}).txt(options.author ? options.author : "admin");
      ...
      status = item.ele("wp:status", {}).txt(options.status ? options.status : "inherit");
      ...
      postMetaAttachmentImage.ele("meta_value").dat(options.attachementImageAltMeta ? options.attachementImageAltMeta : "");
      attachementImageAltMeta = postMetaAttachmentImage;
      type = item.ele("wp:post_type", {}).txt("attachment");
      return type = item.ele("wp:attachment_url", {}).txt(options.attachmentURL ? options.attachmentURL : "");
    }
  };
