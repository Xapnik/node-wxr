...
module.exports = Importer = (function() {
  ...
  Importer.prototype.addPost = function(post) {
    var cat, catCDATA, category, contentEncoded, contentEncodedCDATA, creator, date, id, description, paragraph, excerptEncoded, excerptEncodedCDATA, i, item, len, name, parent, ref, ref1, results, status, title, type, guid, mediaText, mediaLinks, commentStatus, pingStatus, postName, menuOrder, postPassword, isSticky, postMeta;
    ...
    paragraph = item.ele("wp:paragraph", {}).txt(post.paragraph ? post.paragraph : ""); // TODO: check how to make wp:paragraph commented out
    //TODO: add spacer after paragraph
    name = item.ele("wp:post_name", {}).txt(post.name ? post.name : "");
    mediaText = item.ele("wp:media-text");
    if((mediaLinks = post.media_links) != null ? mediaLinks.length : void 0) > 0){
      for (let index = 0; index < mediaLinks.length; index++) {
        const mediaLink = mediaLinks[index];
        mediaText.ele("wp:paragraph", {}).txt(mediaLink);
      }
    }
    //TODO: spacer block
    ...
    postMeta.ele("meta_value").dat(options.meta ? options.meta : "");
