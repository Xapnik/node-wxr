...
module.exports = Importer = (function() {
  ...
  Importer.prototype.addPost = function(post) {
    ...
    postMeta.ele("wp:meta_value").dat(post.meta ? post.meta : "");

    let categoryTag = null;
    if((ref = post.category) != null) {
      if (ref.slug && ref.title) {
        cat = item.ele("category", {
          domain: "category",
          nicename: ref.slug
        });
        categoryTag = cat.dat(ref.title);
      }
      return categoryTag;
    }

    // if (((ref = post.category) != null ? ref.length : void 0) > 0) {
    //   ref1 = post.category;
    //   results = [];
  ...
  Importer.prototype.addAttachment = function(options) {
    ...
    if (options.attachmentURL) {
      ...
      postMetaAttachedFile.ele("wp:meta_value").dat(options.attachedFileMeta ? options.attachedFileMeta : "");
